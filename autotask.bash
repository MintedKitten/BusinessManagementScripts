...
ACCOUNTFILE=Daily_Accounting/accounting_$YESTERDAY.csv
if [ -f "$ACCOUNTFILE" ]
then
	echo "Accounting already made for yesterday"
	exit 1
fi
STOCKFILE=Daily_Stock/stock_$YESTERDAY.csv
SELLFILE=Daily_Sell/sell_$YESTERDAY.csv
# Report Stock
...
echo "$STOCKREPORT"$'\n'"$SELLREPORT" >> "Daily_Accounting/account_$YESTERDAY.txt"
